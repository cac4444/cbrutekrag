/*
 ...
 - Race-safe worker (R1)
 - Debug-mutex protected (no stderr corruption at 1000+ threads)
 - A1 HTTP-check restored exactly
 - Uses heap for thread array
*/
...
/* =========================================== */
/* GLOBAL DEBUG MUTEX → prevents stderr crash  */
/* =========================================== */
                               ...
                               const char *proxy_ip, uint16_t proxy_port)
{
    ...
    if (r != SSH_OK) {
        if (options->verbose & CBRUTEKRAG_VERBOSE_MODE)
            log_error("[!] Error connecting to %s:%d %s.",
    ...
    if (r == SSH_AUTH_ERROR) {
        log_debug("[!] %s:%d - ssh_userauth_none(): A serious error happened.",
                  hostname, port);
        ssh_disconnect(session);
        ssh_free(session);
        if (proxied_fd >= 0) close(proxied_fd);
    ...
    if (method & (int)SSH_AUTH_METHOD_PASSWORD) {
        ...
        if (r == SSH_AUTH_SUCCESS) {
            ...
            if (options->check_http != NULL) {
                ...
                if (r != SSH_OK) {
                    log_error("Error ssh_channel_open_forward: %s",
                                   ...
                                   const char *proxy_ip, uint16_t proxy_port)
{
    ...
    int ret = bruteforce_ssh_login_proxy(context, hostname, port,
                                         _username, _password,
                                         proxy_ip, proxy_port);
...
/* ====================================================== */
/* PATCHED WORKER (R1) – race-safe + debug-mutex protected */
/* ====================================================== */
...
/* ====================================================== */
/* THREAD SPAWNER (unchanged, just uses patched worker)   */
/* ====================================================== */
...
void btkg_bruteforce_start_proxy(btkg_context_t *context)
{
    ...
    pthread_mutex_lock(&dbg_lock);
    fprintf(stderr, "[DEBUG] Thread array allocated at %p\n", (void*)scan_threads);
    pthread_mutex_unlock(&dbg_lock);
    ...
    for (size_t i = 0; i < options->max_threads; i++) {
        ...
        int ret = pthread_create(&scan_threads[i], NULL,
                                 btkg_bruteforce_worker_proxy,
                                 (void*)context);

        if (ret != 0) {
            log_error("Thread creation failed at %zu: %d", i, ret);

            pthread_mutex_lock(&dbg_lock);
            fprintf(stderr, "[DEBUG] Failed to create thread %zu\n", i);
            pthread_mutex_unlock(&dbg_lock);
    ...
    pthread_mutex_lock(&dbg_lock);
    fprintf(stderr, "[DEBUG] Created %zu/%zu threads\n",
    ...
    pthread_mutex_lock(&dbg_lock);
    fprintf(stderr, "[DEBUG] All threads joined\n");
    pthread_mutex_unlock(&dbg_lock);

    free(scan_threads);

    pthread_mutex_lock(&dbg_lock);
    fprintf(stderr, "[DEBUG] brute_force_proxy finished\n");
    pthread_mutex_unlock(&dbg_lock);
}
